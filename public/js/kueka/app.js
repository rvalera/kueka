...
function KuekaVM() {
	...
	self.top_persons = ko.observableArray();
	self.top_organizations = ko.observableArray();
	self.top_sources = ko.observableArray();
	...
	loadTopPersons = function() {

		$.ajax({
			//async : false,
			url : 'most-important-actors/',
			dataType : "json",
			data : {
				type : 'Person'
			},
	...
	loadTopSources = function() {

		$.ajax({
			//async : false,
			url : 'most-important-sources/',
			dataType : "json",
			data : {
			},
			beforeSend : function() {
			...
			success : function(data) {
				data.content.forEach(function (element) {
					ko_element = ko.mapping.fromJS(element);
					self.top_sources.push(ko_element);
				});	
			}
		});
